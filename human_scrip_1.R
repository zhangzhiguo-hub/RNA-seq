###ballgown包处理数据
{
  ...
  bg_filt_trans = subset(bg_filt,apply(bg_filt,1,max)>10,genomesubset=TRUE)
...
####读出并保存数据
{
  write.table(bg_filt_trans,file = "bg_filt_trans.csv",sep = ",",row.names = T) #列表保存为.csv格式
  write.table(bg_filt_trans,file = "bg_filt_trans.txt",sep = " ",row.names = T) #列表保存为.txt格式
}
getwd()
#设置存储路径
...
##UpSetR包绘图
{
  ...
  upset(data_2,nsets = 10,nintersects = 1000,)
